...
def intervalVectorToAddress(vector):
    ...
    for card in range(1,13):
        for item in enumerate(FORTE[card]):
            num, sc = item
            if sc is None:
...
def seekChordTablesAddress(pcSet):
    '''
    ...
    for indexCandidate in range(len(FORTE[card])):
        ...
        for item in candidates:
            ...
            if dataLinePcs == candidate:
                print("dataLinePcs == candidate", dataLinePcs == candidate)
                if 0 in inversionsAvailable:
...
if __name__ == '__main__':
    # res = addressToCommonNames([3,1,0])
    # print("res", res)
    res1 = addressToCommonNames(seekChordTablesAddress([1, 5, 9]))
    print("res1", res1)